...
const indexStore = defineStore('index', {
  ...
  actions: {
    ...
    removeCata(id: number) {
      ...
      if (id === this.active_cataid) {
        this.active_cataid = this.catalogs.length ? this.catalogs[0].cata_id : null
    ...
    removeMemo(id: number) {
      ...
      if (id === this.active_memoid) {
        this.active_memoid = this.activeMemos.length ? this.activeMemos[0].memo_id : null
  ...
  getters: {
    activeMemos: (state) => state.memos.filter(item => item.cata_id === state.active_cataid!)
  }
})
